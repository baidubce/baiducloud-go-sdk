...
func CorrectEduCreateTask() {
	...
	correctEduCreateTaskRequest := &ocr.CorrectEduCreateTaskRequest{
		Image:             util.PtrString(""),
		Url:               util.PtrString(""),
		OnlySplit:         util.PtrBool(false),
		DisablePreprocess: util.PtrBool(false),
	}
	result, err := client.CorrectEduCreateTask(correctEduCreateTaskRequest)
	if err != nil {
